      ...
      <div className="rounded-2xl bg-slate-50 p-4">
        ...
        {"impliedGross" in result && result.impliedGross != null ? (
          <p className="mt-2 text-2xl font-extrabold text-slate-900">
            {locale === "en" ? "Est. gross" : "ברוטו משוער"}: ₪{result.impliedGross.toLocaleString()}
          </p>
        ) : null}
        <p className="mt-1 text-2xl font-extrabold text-[#6F42F5]">
          {locale === "en" ? "Net" : "נטו"}: ₪{result.net.toLocaleString()}
        </p>
        <ul className="mt-3 space-y-1 text-sm text-slate-600">
          <li>
            {locale === "en" ? "Income tax" : "מס הכנסה"}: ₪{result.tax.toLocaleString()}
          </li>
          <li>
            {locale === "en" ? "National Insurance" : "ביטוח לאומי"}: ₪{result.ni.toLocaleString()}
          </li>
          <li>
            {locale === "en" ? "Health tax" : "מס בריאות"}: ₪{result.health.toLocaleString()}
          </li>
        </ul>
      </div>
    </div>
