...
export function DemoNotice({ locale = "he" }: { locale?: string }) {
  ...
  return (
    <div className="rounded-2xl border border-amber-200 bg-gradient-to-l from-amber-50 to-white px-5 py-4 shadow-sm">
      <p className={`text-sm font-bold text-amber-900 ${locale === "en" ? "text-left" : "text-right"}`}>{copy.demoNoticeTitle}</p>
      <p className={`mt-1 text-sm leading-7 text-amber-800/90 ${locale === "en" ? "text-left" : "text-right"}`}>
        {copy.demoNoticeBody}{" "}
