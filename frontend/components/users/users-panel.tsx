    ...
    <div className="space-y-4">
                ...
                <tr key={u.id} className="hover:bg-[var(--muted)]/30">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {u.role_assignments.map((r) => (
                        <span key={r.id} className="inline-flex items-center gap-1 rounded bg-[var(--accent-muted)] px-2 py-0.5 text-xs">
                          {r.name || r.slug}
                          {canChangeRoles && (
                            <button type="button" className="text-[var(--muted-fg)] hover:text-red-600" onClick={() => removeRoleMutation.mutate({ userId: u.id, roleId: r.id })}>×</button>
                          )}
                        </span>
                      ))}
                    </div>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-2">
                      {canEdit && <button type="button" className="text-xs text-[var(--accent)] hover:underline" onClick={() => resetPasswordMutation.mutate(u.id)}>{t("resetPassword")}</button>}
                      {canEdit && !u.email_verified && (
                        <>
                          <button type="button" className="text-xs text-[var(--accent)] hover:underline" onClick={() => resendVerifyMutation.mutate(u.id)}>{t("resendVerification")}</button>
      ...
      {inviteOpen && (
        <Modal title={t("inviteTitle")} onClose={() => setInviteOpen(false)}>
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); inviteMutation.mutate(); }}>
            <Field label={t("colEmail")}><Input type="email" required value={inviteForm.email} onChange={(e) => setInviteForm({ ...inviteForm, email: e.target.value })} /></Field>
            <Field label={t("firstName")}><Input required value={inviteForm.first_name} onChange={(e) => setInviteForm({ ...inviteForm, first_name: e.target.value })} /></Field>
            <Field label={t("lastName")}><Input required value={inviteForm.last_name} onChange={(e) => setInviteForm({ ...inviteForm, last_name: e.target.value })} /></Field>
            <Field label={t("colRoles")}>
              <select className="h-10 w-full rounded-[var(--radius)] border border-[var(--border)] bg-[var(--background)] px-3 text-sm" value={inviteForm.role_slug} onChange={(e) => setInviteForm({ ...inviteForm, role_slug: e.target.value })}>
                {roles.data?.results.map((r) => <option key={r.id} value={r.slug}>{r.name}</option>)}
              </select>
            </Field>
      ...
      {roleUser && (
        <Modal title={t("assignRoleTitle")} onClose={() => setRoleUser(null)}>
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); assignRoleMutation.mutate(); }}>
            <p className="text-sm text-[var(--muted-fg)]">{roleUser.email}</p>
            <Field label={t("colRoles")}>
              <select className="h-10 w-full rounded-[var(--radius)] border border-[var(--border)] bg-[var(--background)] px-3 text-sm" value={roleSlug} onChange={(e) => setRoleSlug(e.target.value)}>
                {roles.data?.results.map((r) => <option key={r.id} value={r.slug}>{r.name}</option>)}
              </select>
            </Field>
            <div className="flex gap-2 pt-2">
